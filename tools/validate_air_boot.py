...
def main(root: Path) -> None:
    ...
    require('required_version' not in vc, 'Starter carries duplicated operative required_version literal')
    require('required_designation' not in vc, 'Starter carries duplicated operative required_designation literal')
    require('required_cross_file_checks' not in vc, 'Starter carries operative free-form required_cross_file_checks')
    registry = vc.get('deterministic_contract_registry', {})
    require(registry.get('execution_semantics') == 'DETERMINISTIC_PIPELINE' and registry.get('inference_policy') == 'PROHIBITED', 'Starter deterministic contract registry missing')
    ...
    dm = starter.get('compiler_contract', {}).get('deterministic_contract_machine_representation', {})
    require(dm.get('required') is True, 'Starter deterministic contract representation mirror missing')
    ...
    require(executed == 102, f'R1 deterministic boot registry expected 102 checks, got {executed}')
    ...
    print(f"Core={markdown_versions['AIR_CORE_RUNTIME.md']} Control={markdown_versions['AIR_CONTROL_SURFACE.md']} Starter={starter['PROMPT_VERSION']} Handoff={card['schema_version']} template_rev{card['template_revision']} user_rev{card.get('user_revision')} Registry={executed}/{executed}")
