...
OUT=pathlib.Path(sys.argv[2]) if len(sys.argv)>2 else ROOT/'validation/AIR_2.5.0_T12D_SPECIALIST_LAYER_VALIDATION.json'
FOUNDATION_ID='AIR_FOUNDATION_2_5_0_MII_CANDIDATE_SET_002'
...
for pkg,(rel,components,mfn,edges) in PKGS.items():
 ...
 for fn in components:
  ...
  add(f'FOUNDATION.{pkg}.{fn}',fc.get('compatibility_state')=='ALIGNED_TO_AIR_2_5_0_MII_CANDIDATE_SET_002' and fc.get('foundation_candidate_set_identity')==FOUNDATION_ID,f'{fc.get("compatibility_state")!r} / {fc.get("foundation_candidate_set_identity")!r}')
  st=str(objs[fn].get('STATUS',objs[fn].get('status','')))
  add(f'LIFECYCLE.{pkg}.{fn}',not any(t in st for t in STALE) and 'T12D_FOUNDATION_RESEAL_STATIC_VALIDATED' in st,st)
 ...
 add(f'MANIFEST.FOUNDATION.{pkg}',fc.get('compatibility_state')=='ALIGNED_TO_AIR_2_5_0_MII_CANDIDATE_SET_002' and fc.get('foundation_candidate_set_identity')==FOUNDATION_ID,f'{fc.get("compatibility_state")!r} / {fc.get("foundation_candidate_set_identity")!r}')
 st=str(m.get('status',m.get('STATUS','')));add(f'MANIFEST.LIFECYCLE.{pkg}',not any(t in st for t in STALE) and 'T12D_FOUNDATION_RESEAL_STATICALLY_VALIDATED' in st,st)
