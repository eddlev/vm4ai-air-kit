...
def main():
    ...
    migrated=mod.migrate_to_current(src,{'AIR_HANDOFF_CARD':H})['AIR_HANDOFF_CARD']; req(migrated['template_revision']==19,'v073 legacy target template rev'); req(migrated['user_revision']==44,'v073 legacy user revision'); req('card_revision' not in migrated,'v073 legacy field leaked'); req(migrated['migration_state']['source_template_revision'] is None,'v073 invented legacy source template revision'); req(migrated['migration_state']['revision_migration_path']=='LEGACY_2_2_FLOOR_TO_REV19','v073 legacy migration path'); req(migrated['surfaced_object_ledger_state']['positive_execution_authority']=='NONE_HISTORY_ONLY','v073 migrated history gained authority')
    rev18={'AIR_HANDOFF_CARD':copy.deepcopy(H)}; c18=rev18['AIR_HANDOFF_CARD']; c18.pop('template_revision',None); c18.pop('user_revision',None); c18['card_revision']=18; r18=mod.migrate_to_current(rev18,{'AIR_HANDOFF_CARD':H})['AIR_HANDOFF_CARD']; req(r18['template_revision']==19 and r18['user_revision'] is None and 'card_revision' not in r18,'v073 rev18 migration semantics wrong')
