...
def main():
    ...
    req(len(starter['validation_contract']['deterministic_contract_registry']['checks'])==102,'R1 registry lost')
    ...
    req(H.get('template_revision')==20,'v074 Handoff current template revision must be 20')
    req('handoff_mode_state' in H and 'handoff_mode_state' in sm['required_fields'],'v074 Handoff mode carrier missing')
    hms=H['handoff_mode_state']; req(hms.get('mode_contract')=='AIR_HANDOFF_MODE_SELECTION_V1' and hms.get('positive_execution_authority')=='NONE','v074 Handoff mode baseline invalid')
    req(hms.get('request_mode')=='UNRESOLVED' and hms.get('selected_mode')=='UNRESOLVED','v074 template fabricated Handoff mode selection')
    req('HANDOFF_MODE_PROVENANCE_VALID' in sm['validation_registry']['allowed_operators'],'v074 Handoff mode validator missing')
    req(eval_pred(H,preds['HC-COND-HANDOFF-MODE-REV20']),'v074 rev20 Handoff mode condition not triggered')
    ...
    rc=sm['revision_migration_contracts']['REV15_TO_REV16']; req(rc['apply_before_current_required_carrier_check'] is True,'072 order flag false'); rc17=sm['revision_migration_contracts']['REV16_TO_REV17']; req(rc17['apply_before_current_required_carrier_check'] is True,'v072 rev16->17 order flag false'); req(rc17['history_synthesis']=='PROHIBITED','v072 rev16->17 history synthesis not prohibited'); rc18=sm['revision_migration_contracts']['REV17_TO_REV18']; req(rc18['apply_before_current_required_carrier_check'] is True,'v072 rev17->18 order flag false'); req(rc18['history_synthesis']=='PROHIBITED','v072 rev17->18 history synthesis not prohibited'); rc19=sm['revision_migration_contracts']['REV18_TO_REV19']; req(rc19['apply_before_current_required_carrier_check'] is True,'v073 rev18->19 order flag false'); rc20=sm['revision_migration_contracts']['REV19_TO_REV20']; req(rc20['apply_before_current_required_carrier_check'] is True and rc20['history_synthesis']=='PROHIBITED','v074 rev19->20 migration contract invalid'); legacy22=sm['revision_migration_contracts']['LEGACY_2_2_FLOOR_TO_REV19']; req(legacy22['legacy_revision_interpretation']=='USER_REVISION','v073 legacy 2.2 revision semantics wrong'); seq=sm['validation_sequence']; req(seq.index('source card revision detection and applicable revision migration check') < seq.index('required-carrier check after applicable migration'),'072 sequence migration after required carriers'); req(rc['history_synthesis']=='PROHIBITED','072 history synthesis not prohibited')
    ...
    src={'AIR_HANDOFF_CARD':copy.deepcopy(H)}; c=src['AIR_HANDOFF_CARD']; c['SCHEMA_VERSION']=c['schema_version']='2.2.0'; c.pop('template_revision',None); c.pop('user_revision',None); c['card_revision']=44; c['profile_stack']['starter_profile']['PROMPT_VERSION']='2.4.3'
    migrated=mod.migrate_to_current(src,{'AIR_HANDOFF_CARD':H})['AIR_HANDOFF_CARD']; req(migrated['template_revision']==20,'v074 legacy target template rev'); req(migrated['user_revision']==44,'v073 legacy user revision'); req('card_revision' not in migrated,'v073 legacy field leaked'); req(migrated['migration_state']['source_template_revision'] is None,'v073 invented legacy source template revision'); req(migrated['migration_state']['revision_migration_path']=='LEGACY_2_2_FLOOR_TO_REV19_TO_REV20','v074 legacy migration path'); req(migrated['surfaced_object_ledger_state']['positive_execution_authority']=='NONE_HISTORY_ONLY','v073 migrated history gained authority')
    mcap=migrated['surfaced_object_ledger_state']['provenance_capture']; provider_fields=['provider_identity','provider_generation','provider_instance_id','provider_namespace_id','provider_namespace_fingerprint','storage_location_class','provider_authorization_state_at_capture','retention_policy','deletion_policy','credentials_serialized']; req(all(k in mcap for k in provider_fields),'v075 migrated provider capture shape incomplete'); req(all(mcap[k] is None for k in ['provider_identity','provider_generation','provider_instance_id','provider_namespace_id','provider_namespace_fingerprint','storage_location_class']),'v075 migrated legacy provider identity fabricated'); req(mcap['provider_authorization_state_at_capture']=='LEGACY_UNRECORDED','v075 legacy provider authorization state invented'); req(mcap['credentials_serialized'] is False,'v075 migrated credentials serialization allowed')
    rev18={'AIR_HANDOFF_CARD':copy.deepcopy(H)}; c18=rev18['AIR_HANDOFF_CARD']; c18.pop('template_revision',None); c18.pop('user_revision',None); c18['card_revision']=18; r18=mod.migrate_to_current(rev18,{'AIR_HANDOFF_CARD':H})['AIR_HANDOFF_CARD']; req(r18['template_revision']==20 and r18['user_revision'] is None and 'card_revision' not in r18,'v074 rev18 migration semantics wrong')
    rev19={'AIR_HANDOFF_CARD':copy.deepcopy(H)}; c19=rev19['AIR_HANDOFF_CARD']; c19['template_revision']=19; c19.pop('handoff_mode_state',None); c19['schema_manifest']['required_fields']=[x for x in c19['schema_manifest']['required_fields'] if x!='handoff_mode_state']; p19=c19['surfaced_object_ledger_state']['provenance_capture']; p19.update({'provider_identity':'filesystem:legacy19','provider_generation':'legacy-gen','provider_instance_id':'legacy-instance','provider_namespace_id':'legacy-ns','provider_namespace_fingerprint':'legacy-ns-fp','storage_location_class':'LOCAL_DURABLE_FILESYSTEM','provider_authorization_state_at_capture':'AUTHORIZED_LOCAL'}); r19=mod.migrate_to_current(rev19,{'AIR_HANDOFF_CARD':H})['AIR_HANDOFF_CARD']; req(r19['template_revision']==20 and r19['migration_state']['source_template_revision']==19,'v074 rev19 migration target/source wrong'); req(r19['handoff_mode_state']['selection_state']=='NOT_EVALUATED' and r19['handoff_mode_state']['positive_execution_authority']=='NONE','v074 rev19 migration fabricated mode authority'); rp19=r19['surfaced_object_ledger_state']['provenance_capture']; req(rp19['provider_identity']=='filesystem:legacy19' and rp19['provider_generation']=='legacy-gen' and rp19['provider_namespace_id']=='legacy-ns','v075 rev19 provider metadata not preserved')
    req('evaluation_basis' in sm['required_fields'],'v074 runtime Handoff evaluation_basis not required')
    req('handoff_generation_evaluation' not in H,'v074 duplicate root handoff_generation_evaluation carrier present')
    hops=sm['validation_registry']['allowed_operators']; req('HANDOFF_GENERATION_EVALUATION_VALID' in hops and 'PORTABLE_HANDOFF_STATE_VALID' in hops,'v074 runtime Handoff validation operators missing')
    hvr=sm['validation_registry']['rules']['HC-VALIDATE-HANDOFF-MODE-PROVENANCE']; req(hvr.get('operator')=='ALL','v074 runtime composite Handoff validator not ALL'); req([x.get('operator') for x in hvr.get('predicates',[])]==['HANDOFF_GENERATION_EVALUATION_VALID','HANDOFF_MODE_PROVENANCE_VALID','PORTABLE_HANDOFF_STATE_VALID'],'v074 runtime Handoff validator sequence mismatch')
    declared=set(sm['required_fields'])|set(sm['optional_fields']); req(set(H)==declared,'Handoff root manifest not closed')
    rmap=load('catalog/AIR_RUNTIME_ROUTE_MAP.json'); hreg=rmap.get('handoff_runtime_contract_registry',{}); req(hreg.get('live_session_owner_path')=='AIR_SESSION.handoff_durability_state','v074 route root live Handoff durability owner missing'); req(hreg.get('generation_evaluation_serialized_carrier')=='AIR_HANDOFF_CARD.evaluation_basis','v074 route root generation carrier mismatch'); req(hreg.get('receipt_contract',{}).get('formal_air_object') is False,'v074 delivery receipt became formal object'); req(rmap['source_of_truth']['sha256']==hashlib.sha256(core.encode()).hexdigest(),'route map core hash stale')
