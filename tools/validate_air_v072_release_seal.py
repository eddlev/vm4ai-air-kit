...
FOUNDATION='AIR_FOUNDATION_2_6_2_OBJECT_CONTRACT_SET_007'
STATUS='AIR_2_6_2_OBJECT_CONTRACT_SET_007_FIVE_PACKAGE_INDEX_V072_CANDIDATE_STATIC_VALIDATED'
COMPLETE='COMPLETE_FOR_AIR_2_6_2_OBJECT_CONTRACT_SET_007_V072_CANDIDATE_SPECIALIST_CATALOG'
...
def main():
    ...
    req('PROMPT_VERSION: 2.6.2' in core,'Core not 2.6.2'); req('PROMPT_VERSION: 2.6.2' in control,'Control not 2.6.2'); req('PROMPT_VERSION: 2.3.2' in gov,'Governance not 2.3.2'); req(starter['PROMPT_VERSION']=='2.6.2','Starter not 2.6.2'); req(card['schema_version']=='2.3.0' and card['card_revision']==18,'Handoff not schema 2.3.0 rev18')
    ...
    req(rmap['MAP_VERSION']=='1.2.1','Route Map not 1.2.1'); routes={r['route_id']:r for r in rmap['routes']}; req(routes['RT.TASK_SWITCH'].get('execution_semantics')=='DETERMINISTIC_PIPELINE','Task Switch not deterministic in Route Map'); req('RT.TASK_SWITCH' in rmap['deterministic_pipeline_contract']['declared_route_ids'],'Task Switch absent from deterministic route catalog')
    req(idx['INDEX_VERSION']=='1.3.2','Index not 1.3.2'); req(idx['status']==STATUS,'Index candidate status mismatch'); req(idx['catalog_scope']['catalog_completeness_claim']==COMPLETE,'Index completeness mismatch'); req(idx['foundation_compatibility_catalog']['identity']==FOUNDATION,'Index Foundation mismatch'); req(idx['candidate_lifecycle_contract']['current_candidate_state']==CANDIDATE,'Index candidate lifecycle mismatch'); req(idx['validation_state']['behavioral_revalidation']=='PENDING_REPLAYABLE_MODEL_HOST_EVIDENCE','unsupported behavioral PASS'); req(idx['validation_state']['release_publication_state']=='EXTERNAL_RELEASE_STATE_NOT_RUNTIME_AUTHORITY','publication boundary changed')
