...
## Results — models #2–#4 (cross-vendor campaign)

The gate was extended from the original two-vendor plan to a four-vendor
campaign using the identical shipped bundle across all runs.
Dates: 2026-07 [EDDE-CONFIRM: exact dates for runs 2–4 from session
records].

| # | Model | T1 (intact) | T2 (tail truncation) |
|---|-------|-------------|----------------------|
| 1 | ChatGPT 5.5 thinking | PASS | PASS — fail-closed error |
| 2 | Claude Opus 4.8 | PASS | PASS — fail-closed error |
| 3 | Gemini 3.5 thinking | PASS | NO-FIRE — check did not execute; boot proceeded |
| 4 | Grok (Auto) | PASS | NO-FIRE — check did not execute; boot proceeded |

**T1: 4/4.** All four vendors emitted per-file load_integrity before Q1
on the intact bundle.

**T2: 2/4.** The truncation error fired on ChatGPT 5.5 thinking and
Claude Opus 4.8. On Gemini 3.5 thinking and Grok Auto the check did not
execute under partial load, despite the integrity law being present in
the loaded text. The file was byte-identical across all four runs, so
host execution under partial-load conditions is the variable — recorded
as findings register F12 (model-dependent check execution). The same
campaign produced F13 (safety posture thins under partial load).

**Additional cross-vendor observations:** the F11 onboarding-record
fabrication reproduced only on ChatGPT 5.5 thinking, not on Claude,
Gemini, or Grok. The S8 F3.1 dual bridge naming (AIR_RUNTIME_BRIDGE vs
AIR_PRIMED_ONBOARDING) was observed in the wild across vendors on
intact files.

## What these tests do and do not establish

They establish that, on the tested models and dates, the shipped
mechanism detects tail truncation and fails closed on two of four
vendors, discriminates between intact and truncated files rather than
defaulting to a verdict, and stays silent when the law is absent. On
the other two vendors the check did not execute under partial load
(F12): truncation detection must be described as best-effort and
model-dependent, never as guaranteed. They do not establish protection
against deliberate tampering, mid-file loss (F10), model compromise, or
prompt injection, and they are not backend validation. Results
generalize only as far as they are re-tested.
